 ...
 */
public class IpReassembly implements PcapPacketHandler<Object> {
	 ...
	 * reassembled buffer contains an IP header upfront and it suitable for
	 * peering with a packet and then decoding.
	 * 
	 * <pre>
	 *  
	 ...
	 * datagram. The original header is used as a template and then several
	 * fields are reset to reflect the state of reassembled packet. The fields
	 * modified are:
	 * <ul>
	 * <li>Ip4.flags</li>
	 * <li>Ip4.length</li>
	...
	public static class IpReassemblyBuffer extends JBuffer implements Comparable<IpReassemblyBuffer> {
		 ...
		 * bytesCopiedIntoBuffer == ipDatagramLength, where
		 * bytesCopiedIntoBuffer keeps track of the total size of the original
		 * datagram in its entirety (including Ip4 header), then the reassembly
		 * is complete.
		 */
		private int bytesCopiedIntoBuffer = 20;

		/**
		 * Offset where the Ip payload begins in the reassembled IP datagram.
		 * Always constant since our IP header is constant as well.
		 */
		...
		/**
		 * Creates a new buffer for IP fragment reassebly. The buffer appends an
		 * Ip4 header to the front of the buffer. The supplied ip header is only
		 * used as a template and a copy is made. This allows the buffer to
		 * retain all the vital Ip4 information found in the original Ip4
		 * datagram before it was fragmented.
		 * 
		 * @param ip
		 *            ip header of one of the fragments to be used as a template
		...
		/**
		 * Deep copy the supplied Ip4 header to the front of the buffer. Reset
		 * some ip fields to reflect the state of this buffer.
		 * 
		 * @param ip
		...
		private void transferFrom(Ip4 ip) {
			...
			/*
			 * Peer a temporary working Ip4 header to the start of our buffer.
			 * It contains our template Ip4 header data.
			 */
		...
		/**
		 * Adds a IP fragment to the buffer. This fragment is also the last
		 * framgent of the fragment series which carries special information
		 * about the original IP datagram.
		 * 
		 * @param packet
		...
		public void addLastSegment(JBuffer packet, int offset, int length, int packetOffset) {
			 ...
			 * buffer now that we know that we have received the last fragment
			 * and where it ends
			 */
		...
		/**
		 * Adds a IP fragment to the buffer. The fragment data is copied into
		 * this buffer at specified offset from the supplied packet data buffer.
		 * 
		 * @param packet
		 *            a packet buffer containing IP fragment data
		 * @param offset
		 ...
		 * buffers for us according to the timeout timestamp. The oldest buffers
		 * are on top of the queue, while the youngest are at the bottom.
		 */
		 ...
		 * timestamp is still in the future, then it returns false. If the
		 * timestamp is in the past, then true is returned and buffer is
		 * considered timedout.
		 * 
		...
		/**
		 * Returns the working Ip4 header instance found at the front of this
		 * buffer
		 * 
	...
	/**
	 * Default buffer size to allocate for reassembly. Needs to be large enough
	 * to hold the Ip4 header upfront and the contetents of all fragment for a
	 * single fragmented Ip4 datagram.
	 */
	...
	/**
	 * Our example application. Arguments are ignored. Reads 6 packets from file
	 * "tests/test-ipreassembly2.pcap" and reassembles the IP fragments found
	 * into a new ip-only super packet. The new packet contains the Ip4 header
	 * as DLT.
	 * 
	 * @param args
	 *            ignored
	...
	public static void main(String[] args) throws FileNotFoundException {
	 ...
	 PrintWriter pw = new PrintWriter("D:/workspace/output.txt");
		//final PrintWriter pw = new PrintWriter(new File(args[1]));

		try {
			int headerID = JRegistry.register(Data.class);
		...
		Data dt = new Data();
		Pcap pcap =
		Pcap.openOffline("D:/workspace/httpsession.pcap", errbuf);
	//Pcap.openOffline(args[0], errbuf);
		 ...
		 * reassembly buffers and we supply our reassembly handler (our
		 * application) as the recipient of packets from libpcap. To it, we
		 * supply an anonymous handler that receives the reassembly buffers. We
		 * simply convert those to packets and print them out. If the buffer is
		 * incomplete, meaning it was timed out before we received the last IP
		 * fragment, we simply report the event as an warning.
		 */

		pcap.loop(50, new IpReassembly(100 * 1000, new IpReassemblyBufferHandler() {
			...
			public void nextIpDatagram(IpReassemblyBuffer buffer) {
				...
				if (buffer.isComplete() == false) {
					 ...
					 * original Ip4 datagram payload. We peer the packet's data
					 * buffer to the reassmbly buffer, starting at the Ip4
					 * header.
					 */
					packet.peer(buffer);
					...
					/*
					 * Decode the packet. We know the first header is the Ip4
					 * header.
					 */
					...
					PacketHandler PcHan = new PacketHandler();

					// System.out.println(pfa.get(22).toString());

					try {

						PcHan.nextPacket(packet, pw);
						// pw.println("++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++");
					} catch (Exception e) {
	...
	/**
	 * Timeout queue to which all new reassembly buffers are added. The queue is
	 * prioritized according to timeout timestamp of each buffer.
	 */
	 ...
	 * 
	 * @param timeout
	 *            sets the default amount of time in millis, before reassembly
	 *            buffers are timedout
	 * @param handler
	...
	/**
	 * Process an Ip4 fragment. Fragment is copied into appropriate reassembly
	 * buffer
	 * 
	 * @param packet
	 *            Ip4 fragment packet
	...
	private IpReassemblyBuffer bufferFragment(PcapPacket packet, Ip4 ip) {
		 ...
		 * packet buffer, while ip.offset() is the Ip4.offset field which is the
		 * fragment offset into the overall datagram, in multiples of 8 bytes
		 */
	...
	/**
	 * Process an Ip4 fragment. Fragment is copied into appropriate reassembly
	 * buffer. This is also a special fragment as its the last fragment of the
	...
	/**
	 * Retrieves a reassembly buffer for this particular Ip packet. The supplied
	 * Ip4 header is used to determine if a buffer already exists and if not a
	 * new one is created.
	 * 
	 * @param ip
	...
	/**
	 * Catch incoming packets from libpcap and if they are Ip packets reassemble
	 * them.
	 * 
	 * @param packet
	...
	public void nextPacket(PcapPacket packet, Object user) {
		...
		if (packet.hasHeader(ip)) {
			...
			/*
			 * Our crude timeout mechanism, should be implemented as a separate
			 * thread
			 */
			timeoutBuffers();
		}
	...
	/**
	 * Check the timeout queue and timeout any buffers that are timedout. The
	 * timed out buffer are dispatched, incomplete to the user's handler.
	 * Buffers that are still on the queue, are incomplete but have not timedout
	 * yet are ignored.
	 */
